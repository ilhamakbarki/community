...
class Settings(core.Settings):
    ...
    def server_url(self):
        return (
            "https://staging.barqfleet.com"
            if self.test_mode
            else "https://barqfleet.com"
        )

    @property
    def tracking_url(self):
        return "https://tracking.barqapp.com/?trackingId={}"
...
def login(settings: Settings):
    ...
    result = lib.request(
        url=f"{settings.server_url}/api/v1/merchants/login",
        method="POST",
        headers={
    ...
    messages = error.parse_error_response(response, settings) if response.get("errors") is not None else []
    if any(messages):
